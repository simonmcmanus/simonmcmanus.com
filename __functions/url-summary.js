...
export const getMeta = async(url, markup, tags) => {
    ...
    const messages = {
        messages: [
            ...
            {
                role: 'user',
                content: `based on the content of ${url} generate a list of up to 6 most relevant tags,
                 The tags shold use uk spelling ideally single words but use hyphons as a seperator 
                 when necessary. Try to use tags that are already used, preferring single words.  
                 Return the list in the json using a tag property`
            },
            {
                role: 'user',
